...
- **Env**: `MERCADOPAGO_ACCESS_TOKEN` (required for paid checkout; production: Firebase Secret Manager via `SecretParam` on `api` in `main.py`; local: `backend_api/.env`), `MERCADOPAGO_API_BASE_URL` (optional, default `https://api.mercadopago.com`).
