...
def list_payments_endpoint(
    ...
    params = PaymentQueryParams(user_id=user_id, order_id=order_id, status=status, limit=limit, offset=offset)
    items = list_payments(repo, params)
    return [p.model_dump(mode="json") for p in items]


@router.get("/{id}")
def get_payment_endpoint(id: str, repo=Depends(get_payment_repository)):
    try:
...
def create_payment_endpoint(data: CreatePaymentInput, repo=Depends(get_payment_repository)):
    return create_payment(repo, data, get_timestamp()).model_dump(mode="json")


@router.put("/{id}")
@router.patch("/{id}")
def update_payment_endpoint(id: str, data: UpdatePaymentStatusInput, repo=Depends(get_payment_repository)):
    try:
...
def update_payment_status_endpoint(id: str, data: UpdatePaymentStatusInput, repo=Depends(get_payment_repository)):
    try:
        return update_payment_status(repo, id, data.status).model_dump(mode="json")
    except PaymentNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
